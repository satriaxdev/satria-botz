...
// Middleware - PERBAIKAN: Pastikan CORS dan JSON parsing diletakkan di awal
app.use(cors());
...
// ✅ FIXED PAIRING SYSTEM - PERBAIKAN BESAR: Hanya pakai QR code untuk kedua metode
app.post('/api/pair/qr', async (req, res) => {
    try {
        ...
        const sock = makeWASocket({
            auth: state,
            version,
            printQRInTerminal: true, // Biar bisa lihat QR di console
            logger: { level: 'warn' },
...
app.get('/api/health', async (req, res) => {
    try {
        // Test AI connection dengan pertanyaan sederhana
        const testPrompt = "Halo, perkenalkan dirimu secara singkat!";
        const aiResponse = await callGeminiAI(HUTAO_CONFIG.prompt + `\n\nUser: "${testPrompt}"`);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📱 Pairing methods: QR Code & Phone Number (Both use QR)`);
});
