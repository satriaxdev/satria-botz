...
// PERBAIKAN: Check server health dengan timeout yang lebih baik
async function checkServerHealth() {
    try {
        ...
        if (error.name === 'AbortError') {
            updateStatus('Server Timeout', 'error');
            showError('Server tidak merespons sayang~ Coba refresh halaman! 💫');
        } else {
            updateStatus('Server Offline', 'error');
            showError('Hu Tao lagi gangguan nih sayang~ Coba refresh halaman! 💫');
        }
    } finally {
        hideLoading();
    }
...
// PERBAIKAN: Generate QR Code dengan error handling yang lebih baik
async function generateQRCode() {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            showError('Timeout: Server tidak merespons sayang~ Coba lagi ya! 💫');
        } else {
...
async function requestPairingCode() {
    ...
    if (phoneNumber.length < 9) {
        showError('Nomor WhatsApp terlalu pendek sayang~ Minimal 9 digit! 😘');
        elements.phoneNumber.focus();
        return;
    }
...
function startStatusChecking() {
    ...
    state.statusCheckInterval = setInterval(async () => {
        ...
        try {
            ...
            // Jika masih waiting, biarkan terus check
        } catch (error) {
